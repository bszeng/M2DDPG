...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", default="MC", help='Algorithm (default: DDPG)')
    ...
    file_dir = f"eps{args.num_eps}_{args.experiment_type}/MC"
    ...
    for eps in range(int(args.num_eps)):
        ...
        print(f"[{datetime.now()}] MC lr:{args.lr}| "
            f"Eps:{eps} | "
            f"Avg Episode Reward: {np.mean(eps_rewards):.6f} ")

    np.save(f"./Learning Curves/{file_dir}/{args.policy}_{file_name}_episode_{args.num_eps}_DDPG", instant_rewards)
