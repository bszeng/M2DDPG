...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", default="Meta", help='Algorithm (default: DDPG)')
    ...
    parser.add_argument("--mode", default="train", choices=["validation", "train"], help="Mode of operation (default: validation)")
    ...
    if mode == 'train':
        ...
        for meta_s, inner_s, tbs in itertools.product(meta_s_list, inner_s_list, tbs_list):
            ...
            meta_agent = DDPG_Meta.DDPG(**kwargs) 
                
            meta_agent.meta_train(
                restored_buffer, 
                meta_steps = meta_s, 
                inner_steps = inner_s, 
                task_batch_size = tbs,
        ...
        for meta_s, inner_s, tbs in itertools.product(meta_s_list, inner_s_list, tbs_list):
            ...
            for eps in range(int(args.num_eps)):
                ...
                for t in range(int(args.num_time_steps_per_eps)):
                    ...
                    if replay_buffer.size > args.batch_size * 2:
                        # Train the agent
                        agent.update_parameters(replay_buffer, args.batch_size)

                    # print(f"Meta: meta_steps={meta_s}, inner_steps={inner_s}, task_batch_size={tbs} | Time step: {t + 1} Episode Num: {episode_num + 1} Reward: {reward:.3f} Buffer Size: {replay_buffer.size} Seed: {args.seed} time: {datetime.now()}")
            ...
            file_name = f"{args.num_antennas}_{args.num_RIS_elements}_{args.num_users}_{args.power_t}_{args.lr}_{args.decay}" # 
            np.save(f"./Learning Curves/{file_dir}/{args.policy}_{file_name}_episode_{args.num_eps}_DDPG_{meta_record}", instant_rewards)
